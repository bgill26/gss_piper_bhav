WITH gm AS (
    ...
    -- AND merchant_type in ('CONVENIENCE')

    GROUP BY year(DATE(datestr))
),
...
ue_data AS (
    ...
    and business_type in ('CONVENIENCE')
    GROUP BY 1
)
...
SELECT
    ...
    ROUND(ue_sales, 2) AS ue_sales,
    CAST(ROUND((gm.gm_sales / ue_sales) * 100, 2) AS VARCHAR) || '%' AS category_share
FROM gm
JOIN ue_data ue
    ON gm.year = ue.year
